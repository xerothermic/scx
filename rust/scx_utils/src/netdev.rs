...
impl NetDev {
    ...
    pub fn apply_cpumasks(&self) -> Result<()> {
        for (irq, cpumask) in self.irqs.iter() {
            let irq_path = format!("/proc/irq/{irq}/smp_affinity_list");
            fs::write(irq_path, cpumask.to_cpulist())?
        }
        Ok(())
    }
...
/// Write a shell script to @path that restores the original smp_affinity_list
/// for all netdev IRQs captured at the time @netdevs was built.  The file is
/// self-contained so it can be run with `sh <path>` as a recovery step.
pub fn write_netdev_irq_restore_script(
    netdevs: &BTreeMap<String, NetDev>,
    path: &Path,
) -> Result<()> {
    let mut f = fs::File::create(path)?;
    writeln!(f, "#!/bin/sh")?;
    writeln!(f, "# Generated by scx_layered --netdev-irq-balance")?;
    writeln!(
        f,
        "# Restores original smp_affinity_list for netdev IRQs."
    )?;
    writeln!(f, "# Run with:  sh {}", path.display())?;
    for (iface, netdev) in netdevs {
        writeln!(f, "\n# {iface}")?;
        for (irq, cpumask) in &netdev.original_irqs {
            let cpulist = cpumask.to_cpulist();
            writeln!(
                f,
                "printf '%s\\n' '{cpulist}' > /proc/irq/{irq}/smp_affinity_list"
            )?;
        }
    }
    Ok(())
